...
import { LogOut, Building2, X, LayoutDashboard, Users, Briefcase, Clock, CalendarDays, DollarSign, UserPlus, TrendingUp, GraduationCap, FileText, Gift, Timer, BarChart3, UserCircle, Bell, Settings, Shield } from 'lucide-react'
  ...
  return (
    <div className="flex flex-col h-full">
      ...
      <nav className="flex-1 overflow-y-auto py-3 px-2 space-y-0.5">
        {items.map(item => (
          <NavLink
            ...
            className={({ isActive }) =>
              `flex items-center gap-3 px-3 py-2.5 rounded-full text-slate-300 hover:bg-slate-800 hover:text-white transition-all text-sm font-medium ${
      ...
      <div className="border-t border-slate-700 p-3 flex-shrink-0">
        {collapsed ? (
          <button className="text-slate-300 hover:text-red-400 transition-colors p-2 mx-auto flex" title="Logout">
          ...
          <div className="flex items-center gap-2.5">
            ...
            <button className="text-slate-300 hover:text-red-400 transition-colors p-1 flex-shrink-0" aria-label="Logout">
              <LogOut className="w-4 h-4" />
            </button>
          </div>
        )}
